...
public class MainUi {
	...
	@Autowired
	private Bank_AdminService bankRepresentativeService;
	@Autowired
	...
	private void login() {// to enter login details
		...
		do {
			...
			try {
				...
				if ((Pattern.matches("^[0-9]+$", tempUci))
						&& main.customerService.checkUciList(new BigInteger(tempUci))) {
					do {
						System.out.println("Password");
						if (keyboardInput.readLine().length() > 0) {
							System.out.print("Welcome");
							System.out.print(" " + main.customerService.returnName(new BigInteger(tempUci)));
	...
	private void addOrDeleteCreditCard() {
		...
		try {
			...
			for (CreditCard card : main.cardService.showCardDetails(uci)) {
				if (card.getCardStatus().equals(Status.PENDING)) {
					count1++;
					if (count1 == 1) {
						System.out.println("\nYour Pending Credit Cards are:");
						System.out.println(message5);
						System.out.printf("%20s %20s %20s", "CREDIT_CARD_NUMBER", "NAME_ON_CARD", "EXPIRY_DATE" + "\n");
						System.out.println(message5);
					}
					System.out.printf("%18s %18s %23s", card.getCardNumber(), card.getNameOnCard(),
							card.getDateOfExpiry() + "\n");
					System.out.println(message5);
					if (!card.getAdminRemarks().contentEquals(" "))
		...
		do {
			...
			case 1:
				...
				try {
					...
					String cardNumberConfirm = keyboardInput.readLine();
					while (!(main.cardService.validateCardNumber(cardNumberConfirm)
							&& cardNumber.equals(cardNumberConfirm))) {
						System.out.println(
								"Both Credit Card numbers do not match.\nPlease enter the Credit Card Number again.");
						cardNumberConfirm = keyboardInput.readLine();
					}
					BigInteger creditCardNumber = new BigInteger(cardNumber);
					card.setCardNumber(creditCardNumber);

					do {
						System.out.println("Please enter a valid Name on your CreditCard ");
	...
	private void addOrModifyBeneficiary() {
		...
		try {
			for (Beneficiary beneficiary : main.beneficiaryAccountService.showBeneficiaryAccount(uci)) {

				if (beneficiary.getStatus().equals(Status.PENDING)) {
					count1++;
					if (count1 == 1) {
						System.out.println("\nYour Pending beneficairies are:");
						System.out.println(message6);
						System.out.printf("%20s %20s %20s %20s %20s", "BENEFICIARY_NAME", "ACCOUNT_NUMBER", "BANK_NAME",
			...
			for (Beneficiary beneficiary : main.beneficiaryAccountService.showBeneficiaryAccount(uci)) {
				...
				if (beneficiary.getStatus().equals(Status.ACTIVE)) {
					count2++;
					if (count2 == 1) {
			...
			for (Beneficiary beneficiary : main.beneficiaryAccountService.showBeneficiaryAccount(uci)) {
				...
				if (beneficiary.getStatus().equals(Status.BLOCKED)) {
					count3++;
					if (count3 == 1) {
						System.out.println("\nYour Active beneficairies are:");
						System.out.println(message6);
		...
		case 1:
			...
			if (0 <= ordinal && CustomerUi.values().length > ordinal) {
				...
				case MYACCOUNTINIBS:
					...
					break;
				case MYACCOUNTINOTHERBANKS:
					addBeneficiary(choice);
					break;
				case OTHERSACCOUNTINOTHERBANKS:
					addBeneficiary(choice);
					break;
				case OTHERSACCOUNTINIBS:
					addBeneficiary(choice);
					break;
				}
			} else {
			...
			if (0 != count2) {
				...
				do {
					...
					try {
						...
						} while (!main.beneficiaryAccountService.validateBeneficiaryAccountNumber(accountNumber1));

						System.out.println("Please enter the account number again");
						String accountNumberConfirm = keyboardInput.readLine();
						while (!main.beneficiaryAccountService.validateBeneficiaryAccountNumber(accountNumberConfirm)
								&& accountNumber1.equals(accountNumberConfirm)) {
							System.out.println(
				...
				do {
					...
					case 1:
						...
						try {
							...
							} while (!main.beneficiaryAccountService
					...
					case 2:
						String ifscNewValue;
						try {
							Beneficiary beneficiary2 = main.beneficiaryAccountService.getBeneficiary(accountNumber);
							if (beneficiary2.getType() == Type.MYACCOUNTINOTHERBANKS
									|| beneficiary2.getType() == Type.OTHERSACCOUNTINOTHERBANKS) {
								do {
									System.out.println("Enter a valid IFSC code(11 characters)");
									ifscNewValue = keyboardInput.readLine();
								} while (!main.beneficiaryAccountService.validateBeneficiaryIfscCode(ifscNewValue));
								beneficiary.setIfscCode(ifscNewValue);
								test = 0;
							} else {
								System.out.println("Modification in ifsc not allowed");
							}
						} catch (IOException | IBSExceptions exception) {
							System.out.println(exception.getMessage());
						}
					...
					case 4:
						try {
							...
							main.beneficiaryAccountService.modifyBeneficiaryAccountDetails(accountNumber, beneficiary);
		...
		case 3:
			...
			if (0 != count2) {
				try {
					...
					} while (!main.beneficiaryAccountService.validateBeneficiaryAccountNumber(deleteAccountNum));

					System.out.println("Please enter the account number again");
					String accountNumberConfirm = keyboardInput.readLine();
					while (!(main.beneficiaryAccountService.validateBeneficiaryAccountNumber(accountNumberConfirm)
							&& deleteAccountNum.equals(accountNumberConfirm))) {
						System.out
								.println("Both Account numbers do not match.\nPlease enter the Account Number again.");
						accountNumberConfirm = keyboardInput.readLine();
					}
					main.beneficiaryAccountService.deleteBeneficiaryAccountDetails(new BigInteger(deleteAccountNum));
	...
	private void addBeneficiary(Type type) {
		...
		try {
			...
			while (!(main.beneficiaryAccountService.validateBeneficiaryAccountNumber(accountNumberConfirm)
					&& accountNumber1.equals(accountNumberConfirm))) {
				System.out.println("Both Account numbers do not match.\nPlease enter the Account Number again.");
				accountNumberConfirm = keyboardInput.readLine();
			}
			...
			do {
				...
				nameInAccount = keyboardInput.readLine();
			} while (!main.beneficiaryAccountService.validateBeneficiaryAccountNameOrBankName(nameInAccount));
			beneficiary.setAccountName(nameInAccount);
			if (type == Type.MYACCOUNTINOTHERBANKS || type == Type.OTHERSACCOUNTINOTHERBANKS) {
				do {
					System.out.println("Please enter a valid IFSC code(11 characters)");
					ifsc = keyboardInput.readLine();
					valid = main.beneficiaryAccountService.validateBeneficiaryIfscCode(ifsc);
				} while (!valid);

				beneficiary.setIfscCode(ifsc);
			} else {
				beneficiary.setIfscCode("IBS12312312");
			}
			if (type == Type.MYACCOUNTINOTHERBANKS || type == Type.OTHERSACCOUNTINOTHERBANKS) {
				do {
					System.out.println("Enter the bank name (case sensitive)");
					bankName = keyboardInput.readLine();
				} while (!main.beneficiaryAccountService.validateBeneficiaryAccountNameOrBankName(bankName));
		...
		try {
			main.beneficiaryAccountService.saveBeneficiaryAccountDetails(uci, beneficiary);
	...
	private void addOrRemoveAutopayments() {
		int count2 = 0;
		try {
			for (AutoPayment autoPayment1 : main.autopaymentservice.showAutopaymentDetails(uci)) {
				count2++;
				if (1 == count2) {
					System.out.println("\nThe added autopayment services for the customer.");
					System.out.println(message7);
					System.out.printf("%20s %20s %20s %20s", "SERVICE_NAME", "START_DATE", "END_DATE", "AMOUNT" + "\n");
					System.out.println(message7);
				}

				System.out.printf("%16s %24s %22s %16s", autoPayment1.getServiceName(), autoPayment1.getDateOfStart(),
						autoPayment1.getDateOfEnd(), autoPayment1.getAmount() + "\n");
				System.out.println(message7);
				count2++;
			}
		} catch (IBSExceptions exception) {
	...
	private void addAutoPayment() {
		...
		serviceProviders = main.autopaymentservice.showIBSServiceProviders();
		System.out.println("\nYour IBS accounts are : ");
		System.out.println(message1);
		System.out.printf("%3s %15s", "SNo.", "ACCOUNTS" + "\n");
		System.out.println(message1);

		try {
			int count1 =1;
			for (AccountBean accountBean : main.accountService.getAccountsOfUci(uci)) {
				System.out.printf("%3s %18s", count1, accountBean.getAccNo() + "\n");
				count1++;
			}
			System.out.println(message1);
		} catch (IBSExceptions exception) {
		...
		if (count == 1) {
			...
			System.out.println(message1);
			System.out.printf("%17s", "SERVICE_LIST" + "\n");
			System.out.println(message1);
			for (ServiceProvider serviceProvider : serviceProviders) {
				System.out.printf("%12s", serviceProvider.getNameOfCompany() + "\n");
			}
			System.out.println(message1);

			while (valid13) {
				System.out.println("\nEnter a valid service name to be registerd.");
				try {
					tempInput = keyboardInput.readLine();
					for (ServiceProvider serviceProvider : serviceProviders) {
						if (serviceProvider.getNameOfCompany().equals(tempInput)) {
							autoPayment.setServiceName(tempInput);
							autoPayment.setServiceProviderId(new ServiceProviderId(serviceProvider.getSpi(), uci));
							valid13 = false;
						}
					}
				} catch (IOException e) {
			...
			} while (!main.cardService.validateDateOfExpiry(mydate));

			String endDate = null;
			do {
				System.out.println("Enter a valid end date in format dd/MM/yyyy");
				try {
					endDate = keyboardInput.readLine();
				} catch (IOException exception) {
					System.out.println(exception.getMessage());
				}
			} while (!main.autopaymentservice.validEndDate(endDate, mydate));
			autoPayment.setDateOfStart(mydate);
			autoPayment.setDateOfEnd(endDate);
			try {
				main.autopaymentservice.autoDeduction(uci, accountNumber, autoPayment);
				System.out.println("\nAutoPayment of service provider: " + tempInput + " added and Rs. " + amount
						+ " will be deducted per month from the date of start till end date." + endDate);
			} catch (IBSExceptions exception) {
				System.out.println(exception.getMessage());
			}

		} else {
			System.out.println("Auto Payment can not be availed without an existing IBS account");
		}
	}
	...
	private void updateAutoPayment() {
		...
		System.out.println("Please enter the service provider name registered");
		try {
			spName = keyboardInput.readLine();
		} catch (IOException exception) {
			System.out.println(exception.getMessage());
		}
		serviceProviders = main.autopaymentservice.showIBSServiceProviders();
		for (ServiceProvider serviceProvider : serviceProviders) {
			if (serviceProvider.getNameOfCompany().equals(spName)) {
		...
		do {
			...
			int choiceToModify = choice(scanner);
			switch (choiceToModify) {
			case 1:
				String amount;
				...
				try {
					...
					do {
						...
						amount = keyboardInput.readLine();
						if (Pattern.matches("^[0-9]+$", amount)) {
							choice = 1;
							amount1 = new BigDecimal(amount);
							autoPayment.setAmount(amount1);
						}
					} while (choice == 0);
				} catch (IOException exception) {
					System.out.println(exception.getMessage());
			...
			case 2:
				...
				try {
					do {
						...
						startDate = keyboardInput.readLine();
					} while (!main.cardService.validateDateOfExpiry(startDate));
					autoPayment.setDateOfStart(startDate);
				} catch (IOException exception) {
					System.out.println(exception.getMessage());
				}
				test = 0;
			...
			case 3:
				...
				String endDate = null;
				try {
					autoPayment2 = autopaymentservice.getAutopayment(new ServiceProviderId(spId, uci));
					do {
						System.out.println("Enter a valid end date");
						endDate = keyboardInput.readLine();
					} while (!main.autopaymentservice.validEndDate(endDate, autoPayment2.getDateOfStart()));
					autoPayment.setDateOfEnd(endDate);
				} catch (IOException | IBSExceptions exception) {
					System.out.println(exception.getMessage());
				}
			...
			case 4:
				try {
					main.autopaymentservice.updateDetails(new ServiceProviderId(spId, uci), autoPayment);
	...
	private void removeAutoPayment() {// removal of autopayment
		...
		try {
			main.autopaymentservice.deleteAutopayment(uci, spId);
	...
	private void showRequests() {
		...
		String uci11 = null;
		int count=1;
		BigInteger uci1 = null;
		try {
			customerRequests = main.bankRepresentativeService.showRequests();
			...
			if (customerRequests.isEmpty()) {
				...
				for (BigInteger customerRequest : customerRequests) {
					System.out.printf("%3s %25s", count ,customerRequest + "\n");
					count++;
				}
				System.out.println(message1 + "---------");
			}
			int choice = 0;
			do {
				System.out.println("Please enter a valid customer id to view individual requests \nEnter 1 to exit");
				uci11 = keyboardInput.readLine();
				if (Pattern.matches("^[0-9]+$", uci11)) {
					choice = 1;
					uci1 = new BigInteger(uci11);
				}
			} while (choice == 0);
		} catch (IOException exception) {
			System.out.println(exception.getMessage());
		...
		if (!uci11.equals("1")) {
			if (customerRequests.contains(uci1)) {
				...
				do {
					int choice1;
					if (!main.bankRepresentativeService.showUnapprovedCreditCards(uci1).isEmpty()) {
						System.out.println("\nCredit Card requests are:");
						System.out.println(message5);
						System.out.printf("%20s %20s %20s %20s", "CREDIT_CARD_NUMBER", "NAME_ON_CARD", "EXPIRY_DATE",
								"TIME_STAMP" + "\n");
						System.out.println(message5);
						for (CreditCard creditCard : main.bankRepresentativeService.showUnapprovedCreditCards(uci1)) {
						...
						System.out.println("\n\nNo credit card requests.");
					}
					if (!main.bankRepresentativeService.showUnapprovedBeneficiaries(uci1).isEmpty()) {
						System.out.println("\nBeneficiary requests are:");
						System.out.println(message6);
						System.out.printf("%20s %20s %20s %20s %20s %20s", "BENEFICIARY_NAME", "ACCOUNT_NUMBER",
								"BANK_NAME", "IFSC_CODE", "ACCOUNT_TYPE", "TIME_STAMP" + "\n");
						System.out.println(message6);

						for (Beneficiary beneficiary : main.bankRepresentativeService
								.showUnapprovedBeneficiaries(uci1)) {

							System.out.printf("%15s %22s %19s %25s %20s %33s", beneficiary.getAccountName(),
					...
					case 1:
						...
						itCredit = main.bankRepresentativeService.showUnapprovedCreditCards(uci1).iterator();
						if (!(itCredit.hasNext())) {
							...
							do {
								...
								case 1:
									try {
										...
										int response = choice(scanner);
										if (response == 1) {
											try {
												System.out.println("Please enter the response.");
												creditCard.setAdminRemarks(keyboardInput.readLine());
											} catch (IOException e) {
												System.out.println(e.getMessage());
											}
										} else
											creditCard.setAdminRemarks(" ");
										main.bankRepresentativeService
					...
					case 2:
						...
						itBeneficiary = main.bankRepresentativeService.showUnapprovedBeneficiaries(uci1).iterator();
								...
								case 1:
									try {
										...
										if (response == 1) {
											try {
												System.out.println("Please enter the response.");
												beneficiary.setAdminRemarks(keyboardInput.readLine());
											} catch (IOException e) {
												System.out.println(e.getMessage());
											}
										} else {
	...
	public static void main(String[] args) {
		...
		main = context.getBean(MainUi.class);
		main.start();
		scanner.close();
		System.out.println();
	}
